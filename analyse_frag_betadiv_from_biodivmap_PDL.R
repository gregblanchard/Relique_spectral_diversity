...
# get cell centroids
xy_cells_centroids_utm <- SpatialPoints(xyFromCell(beta_div$PCoA.1, 1:ncell(beta_div$PCoA.1)), 
                          proj4string=CRS("+proj=utm +zone=58 +south +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"))
xy_cells_centroids_utm <- SpatialPointsDataFrame(xy_cells_centroids_utm, data = data.frame(values(beta_div$PCoA.1)))
# keep only values defined in PCA (i.e. in forest)
xy_cells_centroids_utm <- xy_cells_centroids_utm[!is.na(xy_cells_centroids_utm$values.beta_div.PCoA.1.),]
...
centroids_cells_ok_landscape <- readRDS(paste0(dir_to_save,"centroids_cells_ok_landscape.rds"))

#####################################################################################################
#### Extract environmental variables for each cell ####
#####################################################################################################
#### raster with dimension = windowsize ####
S2_product <- beta_div$PCoA.1
